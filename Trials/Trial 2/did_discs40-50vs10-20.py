...
with open('DISCS-desert-island-discs-OnlyNeededFields_OpenRefined-1940-50s_vs_2010-20s.csv','r',encoding='utf-8-sig') as networkFile: #Whatever your file is called
	networkData = csv.reader(networkFile)

	# Loop 1
	for row in networkData:
		vertex = row[5] #Row of vertex ID (castaway_ref)
		vertexLabel = row[7] #Row of vertex Name (std_name)
		...
		# [...]
		edge = row[5] #Row of edge (std_artist)
...
#Export to CSV
with open("vertices_artist40-50vs10-20.csv",'w') as csvfile1:
	csvwriter = csv.writer(csvfile1)
	csvwriter.writerows(vertices)

with open("edges_artist40-50vs10-20.csv",'w') as csvfile2:
