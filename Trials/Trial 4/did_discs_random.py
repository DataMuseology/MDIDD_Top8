...
with open('DISCS-desert-island-discs-OnlyNeededFields_OpenRefined-RandomizedSelections_PerYear.csv','r',encoding='utf-8-sig') as networkFile: #Whatever your file is called
	...
	for row in networkData:
		vertex = row[7] #Row of vertex ID  (castaway_ref)
		vertexLabel = row[10] #Row of vertex Name (std_name)
		# vertexAttribute1 = row[8] #Row of additional vertex data
		vertexAttribute2 = row[8] #Add additional attributes as needed (std_artist)
		# vertexAttribute1 = row[4]
		# vertexAttribute2 = row[5]
		# vertexAttribute3 = row[6]
		# vertexAttribute4 = row[19]
		# [...]
		edge = row[9] #Row of edge 
...
#Export to CSV
with open("vertices_randomPerYear.csv",'w') as csvfile1:
	csvwriter = csv.writer(csvfile1)
	csvwriter.writerows(vertices)

with open("edges_randomPerYear.csv",'w') as csvfile2:
