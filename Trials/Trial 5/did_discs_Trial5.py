...
with open('26260_DISCS-desert-island-discs-OnlyNeededFields-Trial5_80s.csv','r',encoding='utf-8-sig') as networkFile: #Whatever your file is called
	networkData = csv.reader(networkFile)

	# Loop 1
	for row in networkData:
		vertex = row[8] #Row of vertex ID  (std_artist_id)
		vertexLabel = row[7] #Row of vertex Name (std_artist)
		# vertexAttribute1 = row[8] #Row of additional vertex data
		vertexAttribute2 = row[3] #Add additional attributes as needed 
		...
		v = [vertex, vertexLabel, vertexAttribute2] #[...] Add additional attributes as needed
...
#Export to CSV
with open("vertices_trial5_80s.csv",'w') as csvfile1:
	csvwriter = csv.writer(csvfile1)
	csvwriter.writerows(vertices)

with open("edges_trial5_80s.csv",'w') as csvfile2:
